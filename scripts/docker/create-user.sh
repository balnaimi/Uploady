...
if ! getent group uplody-group > /dev/null 2>&1; then
    addgroup -g "$PGID" uplody-group
fi

# Check if a user with PUID exists; if not, create it
if ! id -u uplody > /dev/null 2>&1; then
    if ! getent passwd "$PUID" > /dev/null 2>&1; then
        adduser -u "$PUID" -G uplody-group uplody > /dev/null 2>&1
    else
